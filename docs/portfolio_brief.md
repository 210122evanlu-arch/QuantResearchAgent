...
| 统计事件研究 | [比亚迪产销快报事件研究](../reports/showcase/byd_event_study.md) | 市场模型、逐日 AR、多窗口 CAR、显著性与污染检查 |
| 市场策略 | [A股市场环境与配置情景](../reports/showcase/a_share_market_strategy.md) | 五信号环境评分、风格/行业矩阵、三情景概率与监测清单 |

六条研究与咨询服务线均已具备端到端离线案例；真实数据刷新、持牌数据覆盖和生产部署成熟度仍按[能力状态表](capability_status.md)区分。
...
- 财务风险报告在初稿后进入独立 IQR，重新计算指标并检查截止日、证据、哈希、报告一致性和保证边界；自动通过不能替代人工签署。
...
> 我设计的不是一个直接回答金融问题的聊天机器人，而是一套研究生产流程。用户提交量化课题、公司研究、行业研究、事件研究、市场策略或风险咨询委托后，系统先结构化研究目标和证据范围，再由模型设计、数据准备和确定性分析引擎完成研究。结论必须经过 Research Committee；发现模型、数据或实验问题时，LangGraph 会定向返回相应节点修改。项目还加入 Debate Gate、证据 ID、截止日控制、财务异常原因代码、独立质量复核和人工签署状态，以及 API、评测和运行诊断。六条服务线均已通过不同案例验证，不依赖单一研究主题。
