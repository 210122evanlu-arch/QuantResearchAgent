...
The bundled offline executor supports the BYD corporate-advisory, Moutai company-research, high-end baijiu industry-research, and BYD statistical event-study showcases. In this endpoint, `enabled=false` means that the bundled HTTP runner does not execute that service line; it does not mean the platform lacks an intake route or report template. See [Capability maturity](capability_status.md). `create_app(runner=...)` accepts an injected runner for production workflows, licensed data, or an external queue. The HTTP contract and job lifecycle remain unchanged.
