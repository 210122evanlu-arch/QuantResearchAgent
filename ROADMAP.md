...
### Platform Round 8：统计事件研究闭环（已完成）

- 将公告事件、证券、事件日、估计窗口、事件窗口和市场基准结构化。
- 实现市场模型、逐日异常收益、CAR、t-stat 与双侧显著性检验。
- 检查重复交易日、数据缺失、事件窗口边界和重叠事件污染。
- 接入委员会定向返工、循环上限、Markdown 报告和 Research Jobs API。
- 增加比亚迪产销快报案例，并严格区分真实公告证据与离线收益夹具。

### Platform Round 9：市场策略闭环（已完成）

- 建立增长、流动性、估值、盈利和风险偏好的有界信号评分。
- 用固定权重和明确阈值识别风险偏好上行、均衡、防御或过渡环境。
- 输出风格与行业配置矩阵、三情景概率、触发条件和监测清单。
- 接入委员会定向返工、循环上限、Markdown 报告和 Research Jobs API。
- 使用统计局、人民银行与上交所公开材料，并明确离线信号不代表实时市场。

### Platform Round 10：财务异常识别与项目质量复核（已完成）

- 建立覆盖盈利质量、营运资本、毛利率、流动性、审计意见和监管事项的透明规则引擎。
- 输出风险评分、原因代码、证据、Owner、Timeline、管理动作和 KPI，不把异常信号表述为舞弊认定。
- 在报告初稿后增加独立质量复核，重新计算指标并检查截止日、证据引用、哈希、报告一致性和保证边界。
- 建立 Pass、Remediation、Blocked 与人工签署待定状态，禁止自动质量检查冒充人工审批。
- 增加可公开分发的合成公司 Demo、API 执行、PRD、UAT 和发布评测。

### Platform Round 11：点时财务与监管风险数据（已完成）

- 将 BaoStock 财务指标按 `pubDate <= as_of_date` 筛选并自动对齐可比报告期。
- 分页采集 CNInfo 公告，区分问询相关披露、监管措施与审计意见证据。
- 在公告标题不足时下载年度报告 PDF，通过页级证据提取标准或非标准审计意见。
- 提供标准化点时表适配器，接入应收、存货、商誉、减值、集中度和研发资本化等完整字段。
- 将规则扩展至 24 项，并建立通用、制造、消费、科技和地产五套版本化行业阈值。
- 增加 `not_available` 状态、加权数据覆盖率和低覆盖 IQR 整改控制，防止缺失数据被解释为低风险。

## Backlog

- 为银行、券商和保险公司建立监管资本、资产质量和流动性专属指标体系。
- 将市场策略接入实时宏观、盈利、估值、资金与市场宽度数据。
- 将统计事件研究接入经许可的真实点时行情和批量事件样本。
